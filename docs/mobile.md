...
This document describes the mobile app's responsibilities, component contracts, and conventions. It is platform-agnostic — implementation details for each platform live in [android.md](android.md) and *(future)* `ios.md`. Both platform implementations follow the naming and structural conventions defined here.
...
|---|---|---|
| Scam hash DB (SQLite) | `phone_hash`, `confidence`, `report_count`, `source`, `updated_at` | Analyzer (crowd signal), Migration service |
| Key-value store | `lastSyncAt`, USER settings, JWT, GUARDIAN config | Migration service, UI, Guardian service |

Phone numbers are normalised to E.164 and hashed (SHA-256) before storage. Raw numbers are never persisted locally.
