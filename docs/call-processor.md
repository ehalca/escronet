...
The Call Processor is split into two layers. This document covers the **Background service** (2.1.1.1). The Analyzer (2.1.1.2) is described in `analyzer.md` *(forthcoming)*.
...
**Actions (in order):**
1. Block / reject the incoming call at the OS level.
2. Show a local notification to USER: "Call blocked — this number is a known scammer" with an **Unblock** action.
3. Emit `CALL_BLOCKED` to backend.
4. If GUARDIAN is configured, backend notifies GUARDIAN.
